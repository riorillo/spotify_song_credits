...
const settings = { headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` } };
...
const getTracks = async (artist, id) => {
    try {
        const res = await fetch(`https://api.spotify.com/v1/search?q=${artist}&type=track&limit=50&offset=${counter}`, settings);
        const data = await res.json();

        //contiene tutte le tracce data la parola chiave specificata col parametro 'artist';
        const tracks = data.tracks.items.filter(item => item.artists.some(element => element.uri === `spotify:artist:${id}`));

        //verifica che le tracce appartengano ad un artista preciso, confrontando l'id e la posizione nell'array 'artists';
        const filteredTracks = tracks.filter(item => item.artists.indexOf(
            item.artists.find(element => element.uri === `spotify:artist:${id}`)) === 0);
...
const getCredits = async (artist, id, initialLoadingValue = 0, concurrency_number) => {
    ...
    const removeDuplicateTracks = foundTracks.filter((item, index) => titles.indexOf(item.name) === index);
    ...
    const getTrackCredits = async (track, trackList) => {
        ...
        try {
            ...
            await page.waitForSelector('#main > div > div.Root__top-container > div.Root__main-view > div.main-view-container > div.os-host.os-host-foreign.os-theme-spotify.os-host-resize-disabled.os-host-scrollbar-horizontal-hidden.main-view-container__scroll-node.os-host-transition.os-host-overflow.os-host-overflow-y > div.os-padding > div > div > div.main-view-container__scroll-node-child > main > section > div.contentSpacing.NXiYChVp4Oydfxd7rT5r.RMDSGDMFrx8eXHpFphqG > div.RP2rRchy4i8TIp1CTmb7 > span > h1');
            await page.click('#main > div > div.Root__top-container > div.Root__main-view > div.main-view-container > div.os-host.os-host-foreign.os-theme-spotify.os-host-resize-disabled.os-host-scrollbar-horizontal-hidden.main-view-container__scroll-node.os-host-transition.os-host-overflow.os-host-overflow-y > div.os-padding > div > div > div.main-view-container__scroll-node-child > main > section > div.contentSpacing.NXiYChVp4Oydfxd7rT5r.RMDSGDMFrx8eXHpFphqG > div.RP2rRchy4i8TIp1CTmb7 > span > h1', { button: 'right' });
            await page.waitForSelector('#context-menu > ul > li:nth-child(2) > button');
            await page.click('#context-menu > ul > li:nth-child(2) > button', { button: 'left' });
            await page.waitForSelector('body > div:nth-child(33) > div > div > div > div.Nw1INlIyra3LT1JjvoqH > div > div:nth-child(3)');

            //preleva i dati relativi agli scrittori e ai produttori;
            const wrote_by = await page.evaluate(() => Array.from(document.querySelectorAll('body > div:nth-child(33) > div > div > div > div.Nw1INlIyra3LT1JjvoqH > div > div:nth-child(3) > span'), e => e.textContent));
            const produced_by = await page.evaluate(() => Array.from(document.querySelectorAll('body > div:nth-child(33) > div > div > div > div.Nw1INlIyra3LT1JjvoqH > div > div:nth-child(4) > span'), e => e.textContent));
            ...
            console.log('An error has occurred at track number', trackList.indexOf(track), track.external_urls.spotify);
    ...
    await Promise.map(removeDuplicateTracks, track => getTrackCredits(track, removeDuplicateTracks), { concurrency: concurrency_number });
    ...
    if (missingTracks.length > 0) {
        ...
        await Promise.map(missingTracks, track => getTrackCredits(track, missingTracks), { concurrency: 8 });
    ...
    fs.writeFileSync(`./data/${artist}_${date}.json`, JSON.stringify(credits, null, 1), "utf-8");
